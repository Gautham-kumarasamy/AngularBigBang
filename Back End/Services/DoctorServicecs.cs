...
*/                }
        ...
        public async Task<string> DeleteDoctors(string username)
        {
            var oldDoctor = await _doctorRepo.Delete(username);
            if (oldDoctor != null)
                return oldDoctor;
            return null;
        }

        public async Task<Doctor> UpdateDoctor(int id, Doctor doctorupdate)
        {
            var users = await _doctorRepo.GetAll();
            var myUser = users.SingleOrDefault(u => u.UserName == doctorupdate.UserName);
            if (myUser != null)
            {
                

                var newUser = await _doctorRepo.Update( id,doctorupdate);
                if (newUser != null)
                {
                    return newUser;
                }
